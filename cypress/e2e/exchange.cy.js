...
function fetchSymbols() {
  return cy.request(BASE_URL_API + "symbols").then((response) => {
    return response.body;
  });
}

function fetchRates(symbol, date) {
  return cy.request(BASE_URL_API + `${date}?base=${symbol}`).then((response) => {
    return response.body;
  });
   ...
   it("verifies that the symbols are fetched from the API correctly and displayed correctly on the input select", () => {
     ...
     fetchSymbols().then((data) => {
       ...
       cy.get("select option:enabled").each(($option, index) => {
         const symbol = symbolsAndDescriptions[index].symbol;
         const description = symbolsAndDescriptions[index].description;
